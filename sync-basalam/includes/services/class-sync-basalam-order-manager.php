...
class SyncBasalamOrderManger
{
    ...
    public static function orderManger(WP_REST_Request $request, $check_sync_status = true)
    {
        ...
        sync_basalam_Logger::debug("دریافت رویداد سفارش: " . json_encode($parsed_params));
        ...
        } elseif (isset($parsed_params['event_id']) && $parsed_params['event_id'] == 3) {
            if ($parsed_params['status'] == '3195') {
                self::completeOrderWoo($parsed_params['more_data']['invoice_id']);
            } elseif ($parsed_params['status'] == '3067' || $parsed_params['status'] == '3233') {
                self::cancelOrderWoo($parsed_params['more_data']['invoice_id']);
            }
        } else {
